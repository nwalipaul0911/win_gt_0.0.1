...
def get_daily_handler(
    ...
    """
    Returns a daily rotating handler writing logs to \
        logs/<category>/<category>-YYYY-MM-DD.log"""
...
session_logger.addHandler(
    get_daily_handler("session", level=logging.INFO, formatter=formatter)
)
session_logger.propagate = False  # Prevent double logging in root

# === Error Logger (for exceptions) ===
error_handler = get_daily_handler(
    "error", level=logging.ERROR, formatter=error_formatter
)
